#!/usr/bin/env python3
"""
Script para probar la nueva funcionalidad de ayuda en anticipos
"""
...
def probar_ayuda_anticipos():
    """Probar la nueva funcionalidad de ayuda en anticipos"""
    print("🔔 PROBANDO NUEVA FUNCIONALIDAD DE AYUDA EN ANTICIPOS")
    print("=" * 60)
    ...
    # 1. Probar carga de la página de anticipos
    print("\n1️⃣ Probando carga de página de anticipos...")
    try:
        response = client.get('/anticipos/')
        if response.status_code == 200:
            content = response.content.decode()
            
            # Verificar que las notificaciones estáticas ya no estén
            if 'Esta acción no se puede deshacer' in content and 'alert alert-info' in content:
            ...
            if 'mostrarAyudaAnticipos()' in content and 'Ayuda sobre Anticipos' in content:
            ...
            print(f"  ❌ Error cargando anticipos: {response.status_code}")
            return False
    ...
    try:
        with open('templates/core/anticipos/list.html', 'r') as f:
            content = f.read()
...
def main():
    """Función principal"""
    print("🔔 PRUEBA DE AYUDA CONTEXTUAL EN ANTICIPOS")
    print("=" * 70)
    
    try:
        # Probar funcionalidad
        ayuda_ok = probar_ayuda_anticipos()
        ...
        if ayuda_ok:
            ...
            print("  1. Ve a: http://localhost:8000/anticipos/")
            print("  2. Observa que ya no hay notificaciones estáticas")
            print("  3. Haz clic en 'Ayuda sobre Anticipos'")
